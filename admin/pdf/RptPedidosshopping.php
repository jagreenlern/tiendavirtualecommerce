...
//	$query = "select  users.name as name, users.billingAddress as ba, users.billingState as bs,users.billingCity as bc,users.billingPincode as bp,orders.id as id,products.productPrice as pp ,orders.quantity as cantidad,sum(products.productPrice)*sum(orders.quantity) as totalfinal  from orders inner JOIN products on orders.productId=products.id inner join users on users.id=orders.userId GROUP by orders.orderDate";
	if(isset($_GET['categoria']) && ($_GET['categoria'])!='' && isset($_GET['subcategoria']) && ($_GET['subcategoria'])!='' && isset($_GET['desde']) && ($_GET['desde'])!='' && isset($_GET['hasta']) && ($_GET['hasta'])!='')
	{
		...
		$query = "select users.name as name, users.billingAddress as ba, users.billingState as bs,users.billingCity as bc,users.billingPincode as bp,orders.id as id,products.productPrice as pp ,orders.quantity as cantidad,sum(products.productPrice)*sum(orders.quantity) as totalfinal from orders inner JOIN products on orders.productId=products.id inner join category on category.id=products.category inner join subcategory on subcategory.id=products.subCategory inner join users on users.id=orders.userId where category.categoryName='$categoria' and  subcategory.subcategory='$subcategoria' and products.postingDate between '$desde' and '$hasta' GROUP by orders.orderDate";
	...
	if(isset($_GET['categoria']) && ($_GET['categoria'])!='' && isset($_GET['subcategoria']) && ($_GET['subcategoria'])!=''){
		$categoria = $_GET['categoria'];
		$subcategoria = $_GET['subcategoria'];
		$query = "select users.name as name, users.billingAddress as ba, users.billingState as bs,users.billingCity as bc,users.billingPincode as bp,orders.id as id,products.productPrice as pp ,orders.quantity as cantidad,sum(products.productPrice)*sum(orders.quantity) as totalfinal from orders inner JOIN products on orders.productId=products.id inner join category on category.id=products.category inner join subcategory on subcategory.id=products.subCategory inner join users on users.id=orders.userId where category.categoryName='$categoria' and  subcategory.subcategory='$subcategoria' GROUP by orders.orderDate";
		$resultado = $connect->query($query);
		//$pdf->Cell(40,6,'hola',1,0,'L');
	...
	if(isset($_GET['categoria']) && ($_GET['categoria'])!=''){
		$categoria = $_GET['categoria'];
	//	$subcategoria = $_GET['subcategoria'];
	$query = "select users.name as name, users.billingAddress as ba, users.billingState as bs,users.billingCityas bc,users.billingPincode as bp,orders.id as id,products.productPrice as pp ,orders.quantity as cantidad,sum(products.productPrice)*sum(orders.quantity) as totalfinal from orders inner JOIN products on orders.productId=products.id inner join category on category.id=products.category inner join subcategory on subcategory.id=products.subCategory inner join users on users.id=orders.userId where category.categoryName='$categoria' GROUP by orders.orderDate";
		$resultado = $connect->query($query);
		//$pdf->Cell(40,6,'hola',1,0,'L');
